...
for index=1:numel(treeNum)
    for jindex=1:numel(splitNum)
        for kindex=1:numel(learner_t)
            ...
            filename =  sprintf('data_collection/learner_split/numbins_256/%d_splitnum_%d_learner_%d_treenum.mat', split,kindex,numTrees);
            filename_fig = sprintf('data_collection/learner_split/numbins_256/%d_splitnum_%d_learner_%d_treenum.fig', split,kindex,numTrees);
            main_guideline;
        end
    end
end
